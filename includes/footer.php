...
?>
<footer class="relative bg-cream pt-10 overflow-hidden">
    <!-- CTA marquee band -->
    <a href="<?= url('contact.php') ?>" class="block border-y border-black/10 py-6 group">
    ...
    <!-- Columns row -->
    <div class="mx-auto max-w-container px-4 py-16">
        <div class="grid gap-12 md:grid-cols-12">
            <!-- Newsletter -->
            <div class="md:col-span-5" data-reveal="up">
                <h3 class="font-display text-2xl font-bold">Newsletter</h3>
                <p class="mt-3 max-w-xs text-sm text-muted">Stay updated with design trends, new templates, and growth insights.</p>

                <form class="mt-6 max-w-sm" data-newsletter>
                    <div class="flex items-center gap-2 rounded-pill border border-black/15 bg-white/60 p-1.5 pl-5 focus-within:border-ink transition-colors">
                ...
                <div class="mt-3 flex items-center gap-2">
                    <?php foreach (site('socials') as $s): ?>
                        <a href="<?= e($s['href']) ?>" aria-label="<?= e($s['label']) ?>"
                           class="grid h-10 w-10 place-items-center rounded-full border border-black/15 text-ink hover:bg-accent hover:border-accent transition-colors">
    ...
    <!-- Giant wordmark -->
    <div class="px-4 pb-8 select-none" aria-hidden="true">
        <h2 data-glitch
            class="glitch font-display font-bold leading-[0.8] text-ink text-center tracking-tighter"
    ...
    <div class="border-t border-black/10">
        <div class="mx-auto max-w-container px-4 py-5 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs uppercase tracking-wide text-muted">
